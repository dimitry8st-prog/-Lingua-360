...
Персональный AI-репетитор английского и испанского языков. Сочетает последовательный маршрут CEFR, цикл «изучи → отработай → примени», RAG по Obsidian, фонетику, рефлексию и интервальное повторение.
...
- пятидневный English Foundation Sprint с короткими текстами;
- эталонное AI-аудио American English в медленном и обычном темпе;
- проверка чтения вслух: распознанный текст, пропуски и замены слов;
- журнал прогресса и ограничение до двух попыток;
- учебный цикл из 10 этапов — от цели до повторения;
- независимая оценка 6 навыков для каждого языка;
- недельный план 70% English / 30% Spanish;
- персонализация урока по сохранённым ошибкам;
- рефлексия и очередь повторения через два дня;
- Telegram-заглушка;
...
- проверка чтения определяет распознанные, пропущенные и заменённые слова, но не выдаёт недостоверный фонемный балл;
...
Проект реализует сбалансированный вариант: системность CEFR и коротких уроков, разговорный цикл Learn → Practice → Apply, понятный видеоконтент, отдельный фонетический тренажёр и мотивационные XP. Это собственная композиция методик; проект не копирует контент или интерфейс сторонних платформ.

Каждый урок проходит 10 этапов: цель, объяснение, RAG-пример, видео/аудирование, голос, AI-диалог, письмо, разбор двух ошибок, рефлексия и интервальное повторение.
